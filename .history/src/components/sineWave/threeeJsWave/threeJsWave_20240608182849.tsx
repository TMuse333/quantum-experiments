...
const ThreeJSWave: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Adjust camera position to the left
    ...
    const createWaveMesh = (amplitude: number, frequency: number, color: number, segments: number) => {
      ...
      for (let i = 0; i < segments; i++) {
        positions[i * 3] = (i / segments) * 10 - 10; // Adjust x-position to the left
        positions[i * 3 + 1] = 0; // y
        positions[i * 3 + 2] = 0; // z
      }
  ...
  return (
    <div className='w-screen bg-red-900'>
      <div className='w-full h-[500px] max-w-[1000px] mr-auto bg-red-400' ref={containerRef}></div>
    </div>
  );
};
