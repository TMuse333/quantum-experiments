...
const CircularWave: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const radius = 50; // Radius of the circular wave
    const waveLength = 600; // Wavelength of the wave
    const amplitude = 5; // Amplitude of the wave
    const frequency = 2; // Frequency of the wave
    const phaseShift = Math.PI / 4; // Phase shift to start the wave from the top
    ...
    const draw = () => {
      ...
      for (let angle = 0; angle < Math.PI * 2; angle += 0.02) {
        ...
        const y = centerY + radius * Math.sin(angle) + amplitude * Math.sin(angle * waveLength + frequency * angle + phaseShift) * Math.sin(angle * frequency * 2);
        if (angle === 0) {
          c.moveTo(x, y);
        } else {
          c.lineTo(x, y);
